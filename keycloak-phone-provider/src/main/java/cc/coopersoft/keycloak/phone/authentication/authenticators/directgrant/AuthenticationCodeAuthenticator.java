...
public class AuthenticationCodeAuthenticator extends BaseDirectGrantAuthenticator {
    ...
    private void validateVerificationCode(AuthenticationFlowContext context, PhoneNumber phoneNumber) {
        getAuthenticationCode(context).ifPresentOrElse(code -> {
            try {
                context.getSession().getProvider(TokenCodeService.class)
                        .validateCode(context.getUser(), phoneNumber, code, TokenCodeType.AUTH);
                context.success();
            } catch (Exception e) {
